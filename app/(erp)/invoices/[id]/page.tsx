...
export default async function InvoiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const canRecordPayment = !isPaid && currentUser?.role !== 'sales'
      ...
      {isOverdue && (
        <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900 rounded-xl px-4 py-3 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-red-600 dark:text-red-500">Due date was {formatDate(invoice.due_date)} · Outstanding: {formatCurrency(invoice.outstanding_balance)}</p>
          </div>
        </div>
      )}
        ...
        <div className="lg:col-span-2 space-y-5">
          <div className="card overflow-hidden">
            <div className="bg-white dark:bg-[#0F1C2E] p-8">
              {/* Doc header — matches INV-000087 exactly */}
              ...
              {/* Bill to + dates — matches INV-000087 */}
              ...
              {/* Totals block — matches INV-000087 */}
              <div className="flex justify-end mb-8">
                <div className="w-64 space-y-2">
                  ...
                  {/* Payment made row — shown in red like INV-000087 */}
                  ...
                  <div key={payment.id} className="px-4 py-3">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        {payment.notes && (
                          <div className="text-xs text-slate-400 mt-0.5">{payment.notes}</div>
                        )}
                        {payment.recorded_by_user && (
                          <div className="text-xs text-slate-400 mt-0.5">
                            by {payment.recorded_by_user.full_name}
                          </div>
                        )}
                      </div>
